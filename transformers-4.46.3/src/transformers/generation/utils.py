...
class GenerationMixin:
    ...
    def sled_greedy_decode(
        ...
        streamer: Optional["BaseStreamer"] = None,
        evolution_rate: Optional[float] = 0.5,
        evolution_scale: Optional[int] = 1,
        evolution_lower_bound: Optional[float] = None,
        **model_kwargs,
        ...
        This method implements a modified greedy decoding algorithm ("SLED greedy decode") that
        leverages intermediate ("premature") layer information to influence token selection.
        In addition to the standard greedy search steps, this function retrieves logits from 
        candidate premature layers (if provided) via the model's hidden states and "evolves" the 
        final (mature) logits by combining them with a weighted contribution from these candidate layers.

        Parameters:
            input_ids (torch.LongTensor): Initial input ids of shape `(batch_size, sequence_length)`.
            mature_layer (int): The layer index to be used as the final (mature) layer.
            generation_config (GenerationConfig): The generation configuration to be used.
            base_layer (Optional[int]): If provided, may be used as an anchor for further adjustments.
            candidate_premature_layers (Optional[List[int]]): List of candidate earlier (premature) layer indices
                from which to derive evolution signals.
            relative_top (float): Relative fraction of the vocabulary to keep when selecting the next token.
            logits_processor (Optional[LogitsProcessorList]): Processors to adjust the logits.
            stopping_criteria (Optional[StoppingCriteriaList]): Criteria used to stop decoding.
            max_length (Optional[int]): Maximum sequence length.
            pad_token_id (Optional[int]): Padding token id.
            eos_token_id (Optional[Union[int, List[int]]]): End-of-sequence token id(s).
            output_attentions (Optional[bool]): Whether to output attentions.
            output_hidden_states (Optional[bool]): Whether to output hidden states.
            output_scores (Optional[bool]): Whether to output token scores.
            output_logits (Optional[bool]): Whether to output raw logits.
            return_dict_in_generate (Optional[bool]): Whether to return a dict (with scores, attentions, etc.).
            synced_gpus (Optional[bool]): For multi-GPU sync.
            streamer (Optional[BaseStreamer]): Optional token streaming object.
            evolution_rate (Optional[float]): Weight to use for candidate logits (default: 0.5).
            evolution_scale (Optional[int]): Multiplicative scale applied after mixing (default: 1).
            evolution_lower_bound (Optional[float]): If set, logits below this threshold will be clamped.
            **model_kwargs: Additional model-specific kwargs.

        Returns:
            Union[GenerateDecoderOnlyOutput, GenerateEncoderDecoderOutput, torch.LongTensor]: 
            Either a torch.LongTensor of generated ids or a GenerateOutput object with additional outputs 
        ...
        pad_token_id = pad_token_id if pad_token_id is not None else generation_config._pad_token_tensor
        output_attentions = output_attentions if output_attentions is not None else generation_config.output_attentions
        output_hidden_states = output_hidden_states if output_hidden_states is not None else generation_config.output_hidden_states
        output_scores = output_scores if output_scores is not None else generation_config.output_scores
        output_logits = output_logits if output_logits is not None else generation_config.output_logits
        return_dict_in_generate = return_dict_in_generate if return_dict_in_generate is not None else generation_config.return_dict_in_generate
        ...
        # Verify that either base_layer or candidate_premature_layers is provided
        if base_layer is None and candidate_premature_layers is None:
        ...
        while self._has_unfinished_sequences(
            ...
            if len(candidate_premature_layers) == 1:
                ...
                new_output_logits = outputs.logits.clone()
            ...
            # Update model_kwargs for the next iteration
            model_kwargs = self._update_model_kwargs_for_generation(
            ...
            # Select next token (argmax for greedy search)
            ...
            if eos_token_id_tensor is not None:
                unfinished_sequences = unfinished_sequences * ~(
                    torch.isin(next_tokens, eos_token_id_tensor.to(next_tokens.device))
                )
            ...
            # Make sure to delete outputs to free memory
            # del outputs really need this???
    ...
    # ) -> Union[GenerateDecoderOnlyOutput, GenerateEncoderDecoderOutput, torch.LongTensor]:
        ...
        while self._has_unfinished_sequences(
            ...
            # Forward pass
            # Handle different model types and their output format
            if hasattr(self, "early_exit_layers") or "early_exit_layers" in inspect.signature(self.forward).parameters:
                # For models that support early exit layers directly
                model_inputs["early_exit_layers"] = early_exit_layers
                dict_outputs, outputs = self(**model_inputs, return_dict=True)
                if base_layer is not None:
                    ...
                    if relative_top > 0.0:
                        ...
                        k = max(int(vocab_size * relative_top), 1)
                        topk_values, topk_indices = torch.topk(mature_logits, k=k, dim=-1)
                        topk_mask = torch.full_like(mature_logits, float('-inf'))
                        topk_mask.scatter_(dim=-1, index=topk_indices, src=topk_values)
                        
                        # Apply mask to base logits
                        base_logits = base_logits.log_softmax(dim=-1)
                        mask = topk_mask < float('-1e3')
                        base_logits.masked_fill_(mask, float('-inf'))
                    
                    # Compute next token logits
                    next_token_logits = mature_logits - base_logits
                else:
                    # Use candidate premature layers
                    stacked_premature_logits = torch.stack([dict_outputs[i][:, -1, :] for i in candidate_premature_layers], dim=0)
                    avg_premature_logits = torch.mean(stacked_premature_logits, dim=0)
                    
                    # Mix with mature logits
                    mature_logits = dict_outputs[mature_layer][:, -1, :]
                    next_token_logits = (1 - evolution_rate) * mature_logits + evolution_rate * avg_premature_logits
                    
                    # Apply scaling
                    next_token_logits = next_token_logits * evolution_scale
                    
                    # Apply lower bound if specified
                    if evolution_lower_bound is not None:
                        next_token_logits = torch.clamp(next_token_logits, min=evolution_lower_bound)
                    
                    # Apply relative top filtering if needed
                    if relative_top > 0.0:
                        vocab_size = next_token_logits.shape[-1]
                        k = max(int(vocab_size * relative_top), 1)
                        topk_values, topk_indices = torch.topk(next_token_logits, k=k, dim=-1)
                        topk_mask = torch.full_like(next_token_logits, float('-inf'))
                        topk_mask.scatter_(dim=-1, index=topk_indices, src=topk_values)
                        next_token_logits = topk_mask
            else:
                # For standard models that return a single output
                outputs = self(**model_inputs, return_dict=True)
                
                # Get mature layer logits (normal logits from output)
                next_token_logits = outputs.logits.clone()[:, -1, :].float()
                next_token_logits = next_token_logits.to(input_ids.device)
                
                # If we have hidden states, use them to get premature layer information
                if output_hidden_states and candidate_premature_layers is not None:
                    hidden_states = (
                    ...
                    for layer_idx in candidate_premature_layers:
                        if layer_idx < len(hidden_states):
                            ...
                            # Compute logits from hidden state (assume self.lm_head exists)
                            candidate_logits = self.lm_head(layer_state)[:, -1, :].float()
                            candidate_logits_list.append(candidate_logits)
                    
                    if candidate_logits_list:
                        # Average the candidate logits
                        avg_candidate_logits = torch.mean(torch.stack(candidate_logits_list, dim=0), dim=0)
                        
                        # Mix with mature logits
                        next_token_logits = (1 - evolution_rate) * next_token_logits + evolution_rate * avg_candidate_logits
...
def _dola_select_contrast(
    ...
    if classifier:
        features = []
        if 'llama' in loaded_model_name or 'mistral' in loaded_model_name:
            features = ['premature_16_top1_token_id', 'premature_16_top1_prob', 'premature_16_top2_token_id','premature_16_top2_prob', 'premature_16_top3_token_id', 'premature_16_top3_prob', 'premature_16_top4_token_id', 'premature_16_top4_prob', 'premature_16_top5_token_id', 'premature_16_top5_prob', 'premature_18_top1_token_id', 'premature_18_top1_prob', 'premature_18_top2_token_id', 'premature_18_top2_prob', 'premature_18_top3_token_id', 'premature_18_top3_prob', 'premature_18_top4_token_id', 'premature_18_top4_prob', 'premature_18_top5_token_id', 'premature_18_top5_prob', 'premature_20_top1_token_id', 'premature_20_top1_prob', 'premature_20_top2_token_id', 'premature_20_top2_prob', 'premature_20_top3_token_id', 'premature_20_top3_prob', 'premature_20_top4_token_id', 'premature_20_top4_prob', 'premature_20_top5_token_id', 'premature_20_top5_prob', 'premature_22_top1_token_id', 'premature_22_top1_prob', 'premature_22_top2_token_id', 'premature_22_top2_prob', 'premature_22_top3_token_id', 'premature_22_top3_prob', 'premature_22_top4_token_id', 'premature_22_top4_prob', 'premature_22_top5_token_id', 'premature_22_top5_prob', 'premature_24_top1_token_id', 'premature_24_top1_prob', 'premature_24_top2_token_id', 'premature_24_top2_prob', 'premature_24_top3_token_id', 'premature_24_top3_prob', 'premature_24_top4_token_id', 'premature_24_top4_prob', 'premature_24_top5_token_id', 'premature_24_top5_prob', 'premature_26_top1_token_id', 'premature_26_top1_prob', 'premature_26_top2_token_id', 'premature_26_top2_prob', 'premature_26_top3_token_id', 'premature_26_top3_prob', 'premature_26_top4_token_id', 'premature_26_top4_prob', 'premature_26_top5_token_id', 'premature_26_top5_prob','premature_28_top1_token_id', 'premature_28_top1_prob', 'premature_28_top2_token_id', 'premature_28_top2_prob', 'premature_28_top3_token_id', 'premature_28_top3_prob', 'premature_28_top4_token_id','premature_28_top4_prob','premature_28_top5_token_id','premature_28_top5_prob','premature_30_top1_token_id','premature_30_top1_prob','premature_30_top2_token_id','premature_30_top2_prob','premature_30_top3_token_id','premature_30_top3_prob','premature_30_top4_token_id','premature_30_top4_prob','premature_30_top5_token_id','premature_30_top5_prob','mature_top1_token_id','mature_top1_prob','mature_top2_token_id','mature_top2_prob','mature_top3_token_id','mature_top3_prob','mature_top4_token_id','mature_top4_prob','mature_top5_token_id','mature_top5_prob']#, 'premature_14_top1_token_id', 'premature_14_top1_prob', 'premature_14_top2_token_id','premature_14_top2_prob', 'premature_14_top3_token_id', 'premature_14_top3_prob', 'premature_14_top4_token_id', 'premature_14_top4_prob', 'premature_14_top5_token_id', 'premature_14_top5_prob',
        elif 'deep' in loaded_model_name or 'codegemma' in loaded_model_name:
            features = ['premature_14_top1_token_id', 'premature_14_top1_prob', 'premature_14_top2_token_id','premature_14_top2_prob', 'premature_14_top3_token_id', 'premature_14_top3_prob', 'premature_14_top4_token_id', 'premature_14_top4_prob', 'premature_14_top5_token_id', 'premature_14_top5_prob', 'premature_16_top1_token_id', 'premature_16_top1_prob', 'premature_16_top2_token_id','premature_16_top2_prob', 'premature_16_top3_token_id', 'premature_16_top3_prob', 'premature_16_top4_token_id', 'premature_16_top4_prob', 'premature_16_top5_token_id', 'premature_16_top5_prob', 'premature_18_top1_token_id', 'premature_18_top1_prob', 'premature_18_top2_token_id', 'premature_18_top2_prob', 'premature_18_top3_token_id', 'premature_18_top3_prob', 'premature_18_top4_token_id', 'premature_18_top4_prob', 'premature_18_top5_token_id', 'premature_18_top5_prob', 'premature_20_top1_token_id', 'premature_20_top1_prob', 'premature_20_top2_token_id', 'premature_20_top2_prob', 'premature_20_top3_token_id', 'premature_20_top3_prob', 'premature_20_top4_token_id', 'premature_20_top4_prob', 'premature_20_top5_token_id', 'premature_20_top5_prob', 'premature_22_top1_token_id', 'premature_22_top1_prob', 'premature_22_top2_token_id', 'premature_22_top2_prob', 'premature_22_top3_token_id', 'premature_22_top3_prob', 'premature_22_top4_token_id', 'premature_22_top4_prob', 'premature_22_top5_token_id', 'premature_22_top5_prob', 'premature_24_top1_token_id', 'premature_24_top1_prob', 'premature_24_top2_token_id', 'premature_24_top2_prob', 'premature_24_top3_token_id', 'premature_24_top3_prob', 'premature_24_top4_token_id', 'premature_24_top4_prob', 'premature_24_top5_token_id', 'premature_24_top5_prob', 'premature_26_top1_token_id', 'premature_26_top1_prob', 'premature_26_top2_token_id', 'premature_26_top2_prob', 'premature_26_top3_token_id', 'premature_26_top3_prob', 'premature_26_top4_token_id', 'premature_26_top4_prob', 'premature_26_top5_token_id', 'premature_26_top5_prob','mature_top1_token_id','mature_top1_prob','mature_top2_token_id','mature_top2_prob','mature_top3_token_id','mature_top3_prob','mature_top4_token_id','mature_top4_prob','mature_top5_token_id','mature_top5_prob']
        elif 'qwen' in loaded_model_name:
            features = [
            'premature_13_top1_token_id', 'premature_13_top1_prob', 
            'premature_13_top2_token_id', 'premature_13_top2_prob', 
            'premature_13_top3_token_id', 'premature_13_top3_prob', 
            'premature_13_top4_token_id', 'premature_13_top4_prob', 
            'premature_13_top5_token_id', 'premature_13_top5_prob', 
            'premature_15_top1_token_id', 'premature_15_top1_prob', 
            'premature_15_top2_token_id', 'premature_15_top2_prob', 
            'premature_15_top3_token_id', 'premature_15_top3_prob', 
            'premature_15_top4_token_id', 'premature_15_top4_prob', 
            'premature_15_top5_token_id', 'premature_15_top5_prob', 
            'premature_17_top1_token_id', 'premature_17_top1_prob', 
            'premature_17_top2_token_id', 'premature_17_top2_prob', 
            'premature_17_top3_token_id', 'premature_17_top3_prob', 
            'premature_17_top4_token_id', 'premature_17_top4_prob', 
            'premature_17_top5_token_id', 'premature_17_top5_prob', 
            'premature_19_top1_token_id', 'premature_19_top1_prob', 
            'premature_19_top2_token_id', 'premature_19_top2_prob', 
            'premature_19_top3_token_id', 'premature_19_top3_prob', 
            'premature_19_top4_token_id', 'premature_19_top4_prob', 
            'premature_19_top5_token_id', 'premature_19_top5_prob', 
            'premature_21_top1_token_id', 'premature_21_top1_prob', 
            'premature_21_top2_token_id', 'premature_21_top2_prob', 
            'premature_21_top3_token_id', 'premature_21_top3_prob', 
            'premature_21_top4_token_id', 'premature_21_top4_prob', 
            'premature_21_top5_token_id', 'premature_21_top5_prob', 
            'premature_23_top1_token_id', 'premature_23_top1_prob', 
            'premature_23_top2_token_id', 'premature_23_top2_prob', 
            'premature_23_top3_token_id', 'premature_23_top3_prob', 
            'premature_23_top4_token_id', 'premature_23_top4_prob', 
            'premature_23_top5_token_id', 'premature_23_top5_prob', 
            'premature_25_top1_token_id', 'premature_25_top1_prob', 
            'premature_25_top2_token_id', 'premature_25_top2_prob', 
            'premature_25_top3_token_id', 'premature_25_top3_prob', 
            'premature_25_top4_token_id', 'premature_25_top4_prob', 
            'premature_25_top5_token_id', 'premature_25_top5_prob', 
            'mature_top1_token_id', 'mature_top1_prob', 
            'mature_top2_token_id', 'mature_top2_prob', 
            'mature_top3_token_id', 'mature_top3_prob', 
            'mature_top4_token_id', 'mature_top4_prob', 
            'mature_top5_token_id', 'mature_top5_prob'
        ]
        else:
            features = ['premature_22_top1_token_id', 'premature_22_top1_prob', 'premature_22_top2_token_id', 'premature_22_top2_prob', 'premature_22_top3_token_id', 'premature_22_top3_prob', 'premature_22_top4_token_id', 'premature_22_top4_prob', 'premature_22_top5_token_id', 'premature_22_top5_prob', 'premature_24_top1_token_id', 'premature_24_top1_prob', 'premature_24_top2_token_id', 'premature_24_top2_prob', 'premature_24_top3_token_id', 'premature_24_top3_prob', 'premature_24_top4_token_id', 'premature_24_top4_prob', 'premature_24_top5_token_id', 'premature_24_top5_prob', 'premature_26_top1_token_id', 'premature_26_top1_prob', 'premature_26_top2_token_id', 'premature_26_top2_prob', 'premature_26_top3_token_id', 'premature_26_top3_prob', 'premature_26_top4_token_id', 'premature_26_top4_prob', 'premature_26_top5_token_id', 'premature_26_top5_prob', 'premature_28_top1_token_id', 'premature_28_top1_prob', 'premature_28_top2_token_id', 'premature_28_top2_prob', 'premature_28_top3_token_id', 'premature_28_top3_prob', 'premature_28_top4_token_id','premature_28_top4_prob','premature_28_top5_token_id','premature_28_top5_prob','premature_30_top1_token_id','premature_30_top1_prob','premature_30_top2_token_id','premature_30_top2_prob','premature_30_top3_token_id','premature_30_top3_prob','premature_30_top4_token_id','premature_30_top4_prob','premature_30_top5_token_id','premature_30_top5_prob','premature_32_top1_token_id','premature_32_top1_prob','premature_32_top2_token_id','premature_32_top2_prob','premature_32_top3_token_id','premature_32_top3_prob','premature_32_top4_token_id','premature_32_top4_prob','premature_32_top5_token_id','premature_32_top5_prob','premature_34_top1_token_id','premature_34_top1_prob','premature_34_top2_token_id','premature_34_top2_prob','premature_34_top3_token_id','premature_34_top3_prob','premature_34_top4_token_id','premature_34_top4_prob','premature_34_top5_token_id','premature_34_top5_prob','premature_36_top1_token_id','premature_36_top1_prob','premature_36_top2_token_id','premature_36_top2_prob','premature_36_top3_token_id','premature_36_top3_prob','premature_36_top4_token_id','premature_36_top4_prob','premature_36_top5_token_id','premature_36_top5_prob','premature_38_top1_token_id','premature_38_top1_prob','premature_38_top2_token_id','premature_38_top2_prob','premature_38_top3_token_id','premature_38_top3_prob','premature_38_top4_token_id','premature_38_top4_prob','premature_38_top5_token_id','premature_38_top5_prob','premature_40_top1_token_id','premature_40_top1_prob','premature_40_top2_token_id','premature_40_top2_prob','premature_40_top3_token_id','premature_40_top3_prob','premature_40_top4_token_id','premature_40_top4_prob','premature_40_top5_token_id','premature_40_top5_prob','mature_top1_token_id','mature_top1_prob','mature_top2_token_id','mature_top2_prob','mature_top3_token_id','mature_top3_prob','mature_top4_token_id','mature_top4_prob','mature_top5_token_id','mature_top5_prob']
        first_row = pd.DataFrame([first_row], columns=features)
        if "bcq" in loaded_model_name:
            state = first_row.values.astype(np.float32).flatten()
            new_predictions = [classifier.select_action(state)]
        else:
