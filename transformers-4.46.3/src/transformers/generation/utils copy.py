        ...
        **model_kwargs,
        ...
        def sled_greedy_decode(
            ...
            **model_kwargs,
            ...
            In addition to the standard greedy search steps as implemented in transformers-4.46, this
            function retrieves logits from candidate premature layers (if provided) via the model’s hidden
            states and “evolves” the final (mature) logits by combining them with a weighted contribution
            from these candidate layers.
            ...
            Parameters:
                ...
                logits_processor (Optional[LogitsProcessorList]): Processors to adjust the logits (as in the standard generate API).
            ...
            Returns:
                Either a torch.LongTensor of generated ids or a dict with additional outputs if return_dict_in_generate is True.
            """
            ...
            unfinished_sequences = torch.ones(batch_size, dtype=torch.long, device=input_ids.device)
            this_peer_finished = False
            # Initialize caches (attention, past_key_values, etc.)
            model_kwargs = self._get_initial_cache_position(input_ids, model_kwargs)

            # Initialize optional outputs if needed
            scores = () if (return_dict_in_generate and output_scores) else None
            raw_logits = () if (return_dict_in_generate and output_scores) else None
            decoder_attentions = () if (return_dict_in_generate and output_attentions) else None
            decoder_hidden_states = () if (return_dict_in_generate and output_hidden_states) else None

            # If encoder-decoder, prepare encoder outputs
            if return_dict_in_generate and self.config.is_encoder_decoder:
                encoder_attentions = model_kwargs["encoder_outputs"].get("attentions") if output_attentions else None
                encoder_hidden_states = (
                    model_kwargs["encoder_outputs"].get("hidden_states") if output_hidden_states else None
            ...
            while self._has_unfinished_sequences(
                ...
                # Forward pass: note that outputs.logits is computed from the final (mature) layer
                outputs = self(**model_inputs, return_dict=True)
                ...
                # Clone final (mature) logits for current step
                next_token_logits = outputs.logits.clone()[:, -1, :].float()
                next_token_logits = next_token_logits.to(input_ids.device)

                # Process logits via processors as usual
                next_token_scores = logits_processor(input_ids, next_token_logits)

                # --- SLED modifications begin ---
                #
                # If candidate premature layer indices are provided and we have hidden states,
                # compute an “evolutionary” candidate by extracting logits from the candidate layers.
                if candidate_premature_layers is not None and output_hidden_states:
                    # Retrieve the tuple of hidden states from the model’s outputs. For encoder-decoder models
                    # use the decoder_hidden_states; for decoder-only models use hidden_states.
                    hidden_states = (
                        outputs.decoder_hidden_states if self.config.is_encoder_decoder else outputs.hidden_states
                    )
                    candidate_logits_list = []
                    for layer_idx in candidate_premature_layers:
                        # Ensure valid index (note: hidden_states[0] is typically the embeddings)
                        if layer_idx < len(hidden_states):
                            layer_state = hidden_states[layer_idx]
                            # Compute candidate logits (assumes that self.lm_head is available to project hidden state to vocab size)
                            # Only take the last token's representation.
                            candidate_logits = self.lm_head(layer_state)[:, -1, :].float()
                            candidate_logits_list.append(candidate_logits)
                    if candidate_logits_list:
                        # Average over the candidate layers
                        avg_candidate_logits = torch.mean(torch.stack(candidate_logits_list, dim=0), dim=0)
                        # Mix the candidate logits with the mature logits using evolution_rate
                        mixed_logits = (1 - evolution_rate) * next_token_scores + evolution_rate * avg_candidate_logits
                    else:
                        mixed_logits = next_token_scores
                else:
                    mixed_logits = next_token_scores
                #
                # Optionally, apply scaling and lower-bound clamp to the mixed logits.
                mixed_logits = mixed_logits * evolution_scale
                if evolution_lower_bound is not None:
                    mixed_logits = torch.clamp(mixed_logits, min=evolution_lower_bound)
                #
                # Optionally restrict token selection to the top relative_top fraction of tokens.
                if relative_top is not None and 0 < relative_top < 1.0:
                    vocab_size = mixed_logits.shape[-1]
                    k = max(int(vocab_size * relative_top), 1)
                    topk_values, topk_indices = torch.topk(mixed_logits, k=k, dim=-1)
                    # Create a mask that sets logits outside the topk to -inf so that argmax only picks among these tokens.
                    topk_mask = torch.full_like(mixed_logits, float('-inf'))
                    topk_mask.scatter_(dim=-1, index=topk_indices, src=topk_values)
                    mixed_logits = topk_mask
                #
                # --- SLED modifications end ---
                #
                # For greedy decoding (do_sample = False) take the argmax.
                next_tokens = torch.argmax(mixed_logits, dim=-1)

                # When finished (e.g., by reaching an EOS) set next tokens to pad_token_id.
                if any(hasattr(criteria, "eos_token_id") for criteria in stopping_criteria):
                    # Here we use unfinished_sequences to ensure finished sequences only output padding.
                    next_tokens = next_tokens * unfinished_sequences + pad_token_id * (1 - unfinished_sequences)

                # Append the new token(s) to input_ids.
                input_ids = torch.cat([input_ids, next_tokens[:, None]], dim=-1)
                if streamer is not None:
                    streamer.put(next_tokens.cpu())

                # Update finished sequences by checking stopping criteria.
                unfinished_sequences = unfinished_sequences & ~stopping_criteria(input_ids, scores)
                this_peer_finished = unfinished_sequences.max() == 0
                cur_len += 1

                # Optionally record outputs for return
                if return_dict_in_generate:
                    if output_scores:
                        scores += (mixed_logits,)
                    if output_hidden_states:
                ...
                # Make sure to delete outputs to free up memory.
                del outputs

            if streamer is not None:
                streamer.end()

            # Return output either as a dict (with extra info) or just the generated ids.
            if return_dict_in_generate:
                if self.config.is_encoder_decoder:
                    return GenerateEncoderDecoderOutput(
                        sequences=input_ids,
